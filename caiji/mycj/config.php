...
$info = '小刀资源采集插件,' . $mycj['site']['install_dir'] . 'addons/mycj/cjv10.php';
if (stristr(file_get_contents($path), $info))
    exit('快捷菜单已存在，请刷新页面后，在 后台首页>>快捷菜单 中查找【小刀资源采集插件】菜单');
elseif (file_put_contents($path, chr(13) . chr(10) . $info, FILE_APPEND))
	exit('快捷菜单添加成功，请刷新页面后，在 后台首页>>快捷菜单 中查找【小刀资源采集插件】菜单');
else
