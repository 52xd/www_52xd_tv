layui.use(['form', 'layer'], function () {
  ...
  $(document).on('click', '#jiekou', function () {
    if (!is_vip) {
      layer.confirm('您当前用的免费版，<br>不支持在这里修改解析接口，<br>VIP版可以自定义修改解析接口！<br><font color="red">如想便捷修改解析接口，请升级成为VIP版！</font><br>PS：如出现此窗口说明插件已失效或插件已更新，请立即反馈站长说明缘由，站长邮箱：admin@52xd.tv', {
        'btn': ['立即登陆', '取消']
      }, function () {
  ...
  $(document).on('click', '.player', function () {
    ...
    if (type == 'play') {
      ...
      $("input[name='play[apis][]']").each(function () {
        $(this).val("https://www.52xd.tv/v/?url=")
      });
    ...
    $.get(path + 'static/player/' + type + '.html', function (data) {
      ...
      switch (type) {
        case 'ckplayerx': var myurl = 'https://cdn.jsdelivr.net/gh/52xd/www_52xd_tv/bofangqi/ckplayer/'; break;
        case 'dplayer': var myurl = 'https://cdn.jsdelivr.net/gh/52xd/www_52xd_tv/bofangqi/DPlay/'; break;
        case 'videojs': var myurl = 'https://cdn.jsdelivr.net/gh/52xd/www_52xd_tv/maccms10/static/player/videojs.html'; break;
        case 'aliplayer': var myurl = 'https://cdn.jsdelivr.net/gh/52xd/www_52xd_tv/maccms10/static/player/aliplayer.html'; break
      }
      layer.confirm('检测到你本地没有' + type + '播放器文件！<br>此播放器源码本站长免费分享给大家！<br>DP及CK两种播放器为后台版<br>V播放器萌芽的有后台，但是需要收费，站长实在找不到！<br>以上播放器源码均有站长收集破解正在使用中！<br>下载源码后上传即可在此安装！不会的联系站长邮箱admin@52xd.tv', {
        icon: 2, title: '提示', btn: ['了解详情', '取消']
      }, function (index, layero) {
          var btn = layero.find('.layui-layer-btn');
          btn.find('.layui-layer-btn0').attr({ href: myurl, target: '_blank' });
          layer.close(index);
          layer.alert('下载源码后上传源码<br>然后在此安装替换播放器<br>不会的联系站长邮箱admin@52xd.tv', function (index) { layer.closeAll() })
      }, function (index) {
        layer.closeAll()
      })
    }, 'json')
